...
static const char* app_ver = "T6A-APP-VER-1.1.3";
...
static unsigned char TriggerStartKey = 0;  /* 0: 360启动按键无触发  1:360启动按键有触发 */
...
#define SPEED_IS_50		50			//360启动按键不触发时，大于此速度关闭AVM
#define SPEED_IS_20		20			//360启动按键触发时，大于此速度，屏幕显示原画面
...
#define SPEED_IS_OVER_50(sp)     (((sp) > SPEED_IS_OVER)?1:0)
#define SPEED_IS_OVER_20(sp)     (((sp) > 20)?1:0)
...
void ParseVehicleInfo(uchar* msg)
{
	...
	if(carInfo.AvmEnableByLamp == 0){//若关闭转向灯功能 则不接收转向灯状态 都按照无转向灯状态处理
		carInfo.Light = 0;
	 }
	
	DBG("carInfo.IsR = %d,  carInfo.Light = %x, carInfo.CarSpeed = %d\n", carInfo.IsR, carInfo.Light, carInfo.CarSpeed);
	#endif
    //printf("carStat ==> %d\n", carInfo.CarStat);
	//printf("light=%d,R=%d,speed=%d,whell=%d\n",carInfo.Light,carInfo.IsR,carInfo.CarSpeed,(float)carInfo.CarSteeringWheel );
}
...
void ParseVideoCalibration(uchar *pMsgData)
{
	...
	SendBuf[0] = cali_state;
	SendPkgStruct(CM_VIDEO_CALIBRATION_ACK,0,0,SendBuf,2);
...
void ParseAdasSwitch(uchar *msg)
{
	...
	{
		printf("LDW打开\n");
		avmCycleSendInfo.LdwSwitchStat = 1;
		TshComm_SetAdasFeatures(EN_TSH_ADAS_LDW,TRUE);
	}
	else{
		/*LDW关闭*/
		avmCycleSendInfo.LdwSwitchStat = 0;
...
/*---------------------------------------------------------------
* 函数原型：void ParseTrunStat(uchar *msg)
* 函数功能：解析并设置转向开关
* 参数说明：
...
* 作者：    liuh
*---------------------------------------------------------------*/
void ParseTrunStat(uchar *msg)
{
	if(msg[0] == 0x01u)
	{
        carInfo.AvmEnableByLamp = 1;
		printf("开启转向功能\n");
	}
	else if(msg[0] == 0x02u)
	{
        carInfo.AvmEnableByLamp = 0;
		printf("关闭转向功能\n");
	}
}

/*---------------------------------------------------------------
* 函数原型：void GetTurnStat(uchar *msg)
* 函数功能：获取转向开关信息
* 参数说明：
...
* 作者：    liuh
*---------------------------------------------------------------*/
void GetTurnStat(uchar *msg)
{
    if(carInfo.AvmEnableByLamp == 1u)
    {
        msg[0] = 0x01;   //turn light is turn off
    }
    else
    {
        msg[0] = 0x02;
...
void ParseSendKey(uchar* pMsgData,uint Hid)
{
	...
	{
		case TEnter: 
			if(avmEnableStat == 0){//avm已经使能的情况下 不算做手动触发
				 TriggerStartKey = 1;
			}
			break;
		...
		case TBack:
			...
			if((carInfo.IsR != 0) || (carInfo.Light != 0)){
				return;
			}
			if(avmEnableStat == 1){
				SetCvbsDispCmd(2);
...
void ParseProtocol(uchar* msgData,uchar cmd)
{
    ...
    case CM_ARM_READY_ACK:                                                        
		...
		SendCamerasFaultState();/*第一次发送摄像头故障状态*/
   		SendMsgBuf[0] = TshComm_GetCaliState();
		SendPkgStruct(CM_VIDEO_CALIBRATION_ACK,0,0,SendMsgBuf,2);//第一次发送标定状态
    ...
    case CM_SET_MODE:
		...
		SendGeneralAck(cmd,Hid,SUCCEED_ACK);		// 发送通用应答
        break;
	...
	case CM_SET_TURN_ENABLE:						// 设置转向开关
		ParseTrunStat(pMsgData);
 	    SendGeneralAck(cmd,Hid,SUCCEED_ACK);//按键的通用应答
		break;

    case CM_GET_TURN_STATUS:
        GetTurnStat(SendMsgBuf);
    ...
    if(size > (uint)MSG_HEAD_SIZE)
    {
        ...
        while((pbuf= arrchr(Srcbuf,MSG_HEAD,(size- (Srcbuf - (uchar*)buf)))) != NULL) //粘包处理
        {
            ...
            if(pbuf[DataLen + (uint)MSG_HEAD_SIZE] != crc)
            {
				...
				PRINTFBUF(DataLen + (uint)MSG_HEAD_SIZE+1, pbuf);
                break;
            }
            else
            {
...
{
	//printf("[liuh>>>ViewChange2D]carInfo.IsR = %d, carInfo.Light = %d\n",carInfo.IsR, carInfo.Light);
	if(carInfo.IsR == 0u)
    {
		if((carInfo.Light ==0u))
        {
	...
	while(DeQueue(&MsgQueue, &msg))
	{
		switch(msg.MsgType)
		{
			case EN_TSH_FIELD_INFORM_ADAS_STATE:   /*通知ADAS状态*/
				if( msg.MsgData  & EN_TSH_ADAS_LDW )
				{
					avmCycleSendInfo.LdwSwitchStat = 1;
				} 
				else
			...
			case EN_TSH_FIELD_INFORM_CAMERAS_STATE:
			 ...
			 case EN_TSH_FIELD_INFORM_CALIRESULT:
			 	if(msg.MsgData == CAMERA_CALI_NONE)
				{
					//avmCycleSendInfo.AvmCaliStat =2; 
					SendBuf[0] = 0x02;
				}
				else
				{
					//avmCycleSendInfo.AvmCaliStat =3;
					SendBuf[0] = 0x03;
					SendBuf[1] = msg.MsgData;
					printf("[liuh>>>>]%s:%d SendBuf[1] = %x\n",SendBuf[1]);
				}
				SendPkgStruct(CM_VIDEO_CALIBRATION_ACK,0,0,SendBuf,2);
			 	break;
...
int Exit_Reverse(void)
{
	return ((lastInfo.IsR == 1u) && (carInfo.IsR == 0));
}

int Turn_Off_lamp(void)
{
	return ((lastInfo.Light != 0) && (carInfo.Light == 0));
}
...
void ProcessViewTransitionLogic(void)
{
	if(avmAllSwitch == 1)
	{	
		if(avmEnableStat == 1) /*使能 则关闭*/
		{
			SetCvbsDispCmd(2); /*总开关关闭      关闭AVM输出 */
		}
		return;
	}
	...
	if(TriggerStartKey == 0)	/*AVM启动按键无触发时*/
	{   		
		...
		if(carInfo.IsR == 1){
			...
			if(carInfo.Light == 0){
				...
				if(carInfo.CarSpeed <= SPEED_IS_50)
				{
					if(avmEnableStat == 0)
					{
						SetCvbsDispCmd(1);
...
void tty1_com_thread(void)
{
	...
	Msg msg;
	printf("%s\n",app_ver);
	carInfo.AvmEnableByLamp = 1;//默认打开转向开关
	CQueueInit(&MsgQueue);
	...
	while(!stopped)
    {
		...
		if(TshComm_GetHostState() == EN_TSH_HOST_STATE_EXCEPTION){
			avmCycleSendInfo.AvmFault = 1;
		}
		else{
			avmCycleSendInfo.AvmFault = 0;
		}
		
		memcpy((void*)&lastInfo,(void*)&carInfo,sizeof(RecCarInfo));
...
void SendHeartBeatPkg(void)
{
	...
	while(!stopped)
	{
		...
		{	
		    ...
		    SendMsgBuf[0] = (uchar)GetCurrViewMode();
			pthread_mutex_lock(&com_lock);
            SendPkgStruct(CM_SEND_DISP_TYPE, 0, 0,SendMsgBuf,1);
			pthread_mutex_unlock(&com_lock);
			usleep(200000);
			if(avmEnableStat == 1)
			{
