...
void __watchdog_feed(void)
{
	...
	iwdg->RLR = 4095; // 12-bit counter, 819ms interval @ 40KHz
...
// For Sleep stage only
void sleep_stimer_start(device_info_t *dev)
{
}
...
void mdelay(u32 ms)
{
	...
	unit = dev->clock.hclk / 9003;/*精确1ms*/
...
void dump_post_message(device_info_t *dev)
{
	...
	dbg_msgv(dev, "\r\nCPU = ARM Cortex-M3, r%dp%d @ %d MHz",
	...
	reg = READ_REG32(0x1ffff7e0); // Flash size register, STM32 UM

	dbg_msgv(dev, "Flash = %dKB, on-chip\r\n", reg & 0xffff);
...
void device_low_power_enter(void)
{
	...
	gpio[0].CRL = GPIOA->CRL;
	gpio[0].CRH = GPIOA->CRH;
	
	gpio[1].CRL = GPIOB->CRL;
	gpio[1].CRH = GPIOB->CRH;

	gpio[2].CRL = GPIOC->CRL;
	gpio[2].CRH = GPIOC->CRH;

	gpio[3].CRL = GPIOD->CRL;
	gpio[3].CRH = GPIOD->CRH;

	gpio[4].CRL = GPIOE->CRL;
	gpio[4].CRH = GPIOE->CRH;
	...
	// watchdog GPIO can't change
	watchdog_gpio_init();

	// PB1, need re-config, has NO-effect!!!
//	GPIOB->CRL &= ~(0x0fu << 4);
	//GPIOB->CRL |= (0x06u << 4); // open-drain output
//	GPIOB->CRL |= (0x02u << 4); // push-pull output
//	GPIOB->BSRR = __BIT(1 + 16); // output LOW

	// PE12 need re-config
//	GPIOE->CRH &= ~(0x0fu << 16);
//	GPIOE->CRH |= (0x02u << 16);
//	GPIOE->BSRR = __BIT(12 + 16);
}

void device_low_power_leave(void)
{
	GPIOA->CRL = gpio[0].CRL;
	GPIOA->CRH = gpio[0].CRH;

	GPIOB->CRL = gpio[1].CRL;
	GPIOB->CRH = gpio[1].CRH;

	GPIOC->CRL = gpio[2].CRL;
	GPIOC->CRH = gpio[2].CRH;

	GPIOD->CRL = gpio[3].CRL;
	GPIOD->CRH = gpio[3].CRH;	

	GPIOE->CRL = gpio[4].CRL;
	GPIOE->CRH = gpio[4].CRH;
}
...
{
#if 1 //(ENABLE_EXTERNAL_WDG | ENABLE_INTERNAL_IWDG | ENABLE_INTERNAL_WWDG)
//	if(get_flag(dev, FLAG_TIMEOUT_WD))
//	{
	//	__watchdog_feed();
		Iwdg_Feed();
//	}
