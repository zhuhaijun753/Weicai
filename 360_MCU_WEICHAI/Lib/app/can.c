...
void enable_err_retrans_mode(u8 is){
	...
	// wait hw ack
	while (((can->MSR & CAN_MSR_INAK) != CAN_MSR_INAK) && (ack != INAK_TIMEOUT))//等待初始化请求确认
	{
		Iwdg_Feed();
		ack++;
	}
	...
	if(is){
		can->MCR &= ~(uint32_t)CAN_MCR_NART; // 错误自动重传 
	...
	/* Request leave initialisation 退出初始化模式 */
	can->MCR &= ~(uint32_t)CAN_MCR_INRQ;
	/* Wait the acknowledge */
	ack = 0;
	while (((can->MSR & CAN_MSR_INAK) == CAN_MSR_INAK) && (ack != INAK_TIMEOUT))//等待初始化退出 确认
	{	
		Iwdg_Feed();
		ack++;
	}
...
void reset_can(void){
	...
	while (((can->MSR & CAN_MCR_RESET) == CAN_MCR_RESET) && (ack != INAK_TIMEOUT))//等待初始化退出 确认
	{	
		Iwdg_Feed();
		ack++;
	}
...
/*
*@brief：初始化can1
*输入：fmt 波特率设置（RTR寄存器）  *filters 过滤器    fmt  波特率  
*/
...
//	can->MCR |= CAN_MCR_TTCM; // enable
	...
	can->MCR |= CAN_MCR_ABOM; // enable 自动退出离线(Bus-Off)管理
//	can->MCR &= ~(uint32_t)CAN_MCR_ABOM; // disable

	/* Set the automatic wake-up mode */
//	can->MCR |= CAN_MCR_AWUM; // enable 
	can->MCR &= ~(uint32_t)CAN_MCR_AWUM; // disable自动唤醒模式  右软件唤醒

	/* Set the no automatic retransmission */
//	can->MCR |= CAN_MCR_NART; // 错误不自动重传 
	can->MCR &= ~(uint32_t)CAN_MCR_NART; // Cause TX-HALT after 3 mailbox full!!!!!!!!!!!!!!!!错误自动重传知直到发送成功

	/* Set the receive FIFO locked mode */
	can->MCR &= ~(uint32_t)CAN_MCR_RFLM; // Disable 在接收溢出时FIFO未被锁定，当接收FIFO的报文未被读出，下一个收到的报文会覆盖原有的报文

	/* Set the transmit FIFO priority */
	can->MCR &= ~(uint32_t)CAN_MCR_TXFP; // Disable优先级由报文的标识符来决定

	/* Set the bit timing register  CAN位时序寄存器  */
	can->BTR = fmt;
	/* Request leave initialisation 退出初始化模式 */
	can->MCR &= ~(uint32_t)CAN_MCR_INRQ;
	...
	can->IER |= CAN_IT_FMP0 | CAN_IT_FMP1;//当FIFO 0\1的FMP[1:0]位为非0时，产生中断。
//	can->IER |= CAN_IT_ERRIE|CAN_IT_BOFIE|CAN_IT_LECIE;//使能错误中断
...
{
	//car_can_deinit_simple(dev); 加上会导致死机
#if ENABLE_CAR_CAN
	can_init(dev,
	#if ENABLE_DIANDONGCHE
		CAN_FMT_NOR_250K,
...
//修改填充当前发送事件型报文的发送次数 根据id name查找
...
/*
	充填发送数据  immediately为立即发送标志位 当 immediately =1 下次无条件立即发送 当immediately = 0 时，按照之前设定的时间循环发送
*/
...
void can_msg_send_handle(device_info_t *dev)
{
	...
	if(get_flag(dev,FLAG_TIMEOUT_SEND_BODY_CAN_10MS)){
		time_flag = 1; //10ms定时到来
	}else{
		time_flag = 0;
	}

	for(i=0; i< (sizeof(g_body_can_send_block)/sizeof(body_can_send_block));i++){
		if(pdata[i].send_flag > 0){ //立即发送
			...
			if(pdata[i].type == EVENT){
				...
				if(pdata[i].send_flag == 1){
					...
					if((pdata[i].cur_time >= 490) || (pdata[i].cur_time == 0)){ //当前要发送的周期事件型报文与上一次或下一次相差不超过10ms  则本次不发送
						return;
					}
					car_can_send(dev, &pdata[i].can_msg);	
				}
			...
			if((50<pdata[i].cur_time)&&(pdata[i].cur_time<300))
			{
				...
				if(dev->did_t.flag)
				{
					dbg_msg(dev, "relay to write did,keep BC520 cycle normal\r\n");
					dev->did_t.flag=0;
