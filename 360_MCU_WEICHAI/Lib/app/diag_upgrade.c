...
//static u8  Upflag=0;  //诊断升级标志位
UPGRADE_T stUpgradeParam = {0XFF, 0, 0, 0, 0, 0};//初始化升级结构体
...
* Author     :
*************************************************************************************************/
static unsigned int  GenerateCrc32_1(u32 *inivalue,u8  *Buffer,u32 Length)
{
    u32  crc;
    u32  i;
...
{
	u8 send_data[8]={0x05,0x71,0x1,0x01,0x00,0x01};//0x78 响应等待
	dev=get_device_info();
	dev->ci->host_ready = 0 ;//开始接受36数据
	...
	dev->ci->state = HOST_UPGRADING ;//开始接受36数据
...
* Author     :
****************************************************************************************************/
static u8 FileCRC_Check(device_info_t *dev)
{	
	u16 len = 256;
	u32 read_len=0;
	u32 crc_value = 0;
	...
	while(read_len < stUpgradeParam.fileTotal)
	{
		...
		if(len > 0)
		{
			...
			{
				para_read_Nbyte( (APP_CODE_BASE+read_len),  update.buff, len);
				}
			else
			{
				para_read_Nbyte( BOOT_CODE_BASE+read_len,  update.buff, len);
				}
			crc_value = GenerateCrc32_1(&crc_value, update.buff, len);		
			read_len += len;
		}
		watchdog_feed(dev);
	}
...
void update_mcu_ver(void)
{
//	device_info_t *dev;
 	u8 buf[6]={0};
	para_read_Nbyte((ADDR_SoftwareVersion+4),buf,6);	
	//	dbg_msg(dev,"test\r\n");
	//注意读出来的值是字符 ASSIC码  转换为%d 减45
	buf[5]+=1;
	if(buf[5]>'9')
	{
		buf[5]='0';
		...
		if((buf[4]>'9')&&(buf[5]=='9'))
		{
			buf[4]='0';
			buf[2]+=1;
			if((buf[2]>'9')&&(buf[4]=='9')&&(buf[5]=='9'))
			{
				buf[2]='0';
				buf[1]+=1;
				if((buf[1]>'9')&&(buf[2]=='9')&&(buf[3]=='9')&&(buf[5]=='9'))
				{
					buf[1]='0';
					buf[2]='0';
					buf[4]='0';
	...
	STMFLASH_Write(ADDR_SoftwareVersion+4,(u16 *)buf,3);
	mdelay(100);
}
...
{
	u32 i=0;
	u32   addr=APP_FLAG_ADDR;/*app flag erase*/
	stUpgradeParam.optStep = 0XFF;
	iflash_unlock();
	i = iflash_page_erase(addr); // clear flag sector
...
{
	UINT8 response_data[8]= {0x04,0x74,0x20,0x04,0x04,0x00,0x00,0x00};//固定格式的回应，1K当做一个包，最大app数据为 0x2C：44kb/1KB。
	
	if(OPT_EREASE_MEMORY_OK == stUpgradeParam.optStep)
	{//4个字节地址，后跟4个字节文件长度(实际升级的数据内容，不包含SID和包编号)
		stUpgradeParam.fileTotal = ((buf[6])<<24)|\
			                       ((buf[7])<<16)|\
			                       ((buf[8])<<8 )|\
			                       (buf[9]);
		stUpgradeParam.update_type =buf[10];
		dbg_msgv(dev, "升级文件大小：%d 个字节\r\n",stUpgradeParam.fileTotal );
		can_msg_Send_SWJ(dev, AVM_Phy_Resp,(u32*) response_data);//等待$36 传输服务
		stUpgradeParam.optStep  = OPT_REQ_DOWNLOAD_OK;//0x01
		stUpgradeParam.bagSeq   = 1; // the first pack  36服务的第一个包
...
void upgrade_transferData(device_info_t *dev,u8 *buf, u16 len)
{
	...
	//3字节包序号改为4字节，高字节在前
	curBagSeq =0;
	curBagSeq |=((u32)buf[3])<<16;
	curBagSeq |=((u32)buf[2])<<8;
	curBagSeq |=buf[1];
	//升级MCU
...
//		Upflag = 0;
	  ...
	  if(0 == Update_WriteAndCheck(dev,&buf[4],curBagSeq,len-4))//len减去包编号占一个字节
...
{	
	u8 response_data[8]   = {0x05,0x71,0x01,0x02,0x02};
...
//	Upflag = 0;
	if(OPT_EXIT_DOWNLOAD_OK == stUpgradeParam.optStep)
	{
		...
		if(1 == FileCRC_Check(dev))
		  {
			response_data[5]= 0x01;/*校验成功*/
			can_msg_Send_SWJ_Up(dev,AVM_Phy_Resp,(u32*)response_data);
			stUpgradeParam.optStep = OPT_REQ_CHECK1_OK;	
			update_mcu_ver();
			dbg_msg(dev, "数据校验成功,升级完成\r\n");
