...
if(dev->ci->can_no_tx==0x00)
{
	...
	{	
		if(i==0x01)
		{
			dbg_msg(dev, "bus err,位填充错\r\n");
			}
		else if(i==0x02)
		{
			dbg_msg(dev, "bus err,格式(Form)错\r\n");
			}
		else if(i==0x03)
		{
		//检测到 ACK错误，自动重发当前失败的帧，持续t(TX_TIMEOUT)=150ms依然ack丢失，则停止发送，重新初始化t(TX_RECOVERY)=150ms在发送。
			//dbg_msg(dev, "bus err,确认(ACK)错\r\n");
			}
		
		else if(i==0x04)
		{
			dbg_msg(dev, "bus err,隐性位错\r\n");
			}
		else if(i==0x05)
		{
			dbg_msg(dev, "bus err,显性位错r\n");
			}
		else if(i==0x06)
		{
			dbg_msg(dev, "bus err,CRC错误\r\n");
			}
		else
		{
	...
	/*	if(get_busoff_sts()==0x01)//boff == 1 ,此时TEC==0xff 发送错误计数器满，进入离线模式
		{
			//快恢复第一次
			car_can_init(dev);
			dev->ci->can_no_tx=0x01;//禁止发送报文
			dev->ci->BusOffQuick=0x01;//进入 慢  恢复  T[BusOffQuick]=100m	
