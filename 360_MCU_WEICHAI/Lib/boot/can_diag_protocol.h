...
//U60 潍柴 ID分配
#define AVM_Phy_Req           (0x743) //诊断设备到控制器的物理请求CAN标识符
#define FUNCATIONAL_REQ_ADDR  (0x7DF) //诊断设备功能寻址请求CAN标识符	
#define AVM_Phy_Resp          (0x74B) //控制器到诊断设备的物理响应CAN标识符
...
typedef struct{
	...
	uint8_t         security_seed[4]; //种子　　分Seed_App   Seed_Bootloader
	...
	uint32_t 	open_securitu_timer; //接收到任何桢时间
...
{//
	u8 HMI_AVMInfo;
	u8 HMI_SetCarColor;
	u8 HMI_SetLampTurnEnable;
	u8 HMI_AVMModeSet;
	u8 HMI_AVMViewTypeSet;
	u8 HMI_AVM_SW;
//
	u8 BCM_StatusIgnition;
	u8 BCM_StatusLamp; 
	u8 BCM_SWStatusShiftRMT;
	//
	u8 ESP_VehicleSpeedValid; 
	u8 SAS_SteeringAngleVD;
	u16 ESP_VehicleSpeed;
	s16 SAS_SteeringAngle;
	//
	u8 EMS_EngineRunningStatue; 
	u8 EMS3_CheckSum;
	//
	u8 TCU_TGSLever; 
	u8 TCU_TGLeverValid;
...
CAN_DIAG_EXT void set_diag_session(uint8_t type);
