...
#include <can.h>
#include <iflash.h>
// 1: use PD8, PD9, use full remap
// 0: use PB10, PB11, no remap
...
static void usart_irq(u32 which)
{
	...
	if(__BIT(5) & usart_dev->uart->SR)//RXNE:???????? (Read data register not empty)
	{
		rb->mem[rb->write] = usart_dev->uart->DR;
		rb->write=(rb->write +1)%USART_RX_BUF_SIZE;
	}

	if(__BIT(3)  & usart_dev->uart->SR)//????,????   ????Err?????,???????? ????????????,????? 
	{
		usart_dev->uart->SR  &= (u16)~((u16)0x0008);//??????
...
void usart_init(device_info_t *dev, u32 which, u32 br, u32 fmt)
{
	...
	switch(which)
	{
		case USART_DEV0:
			...
			rcc->APB2ENR = reg32;
			// config GPA9, GPA10
			IO_INIT_O_AF_PP(GPIOA, 9);
			IO_INIT_I_FLT(GPIOA, 10);
		...
		case USART_DEV1:
			...
			reg32 |= __BIT(18);
			rcc->APB1ENR = reg32;
			IO_INIT_O_AF_PP(GPIOB, 10);
			IO_INIT_I_FLT(GPIOB, 11);
	...
	// enable usart, word length
	usart->CR1 = 0; // reset default value
	reg = fmt;
	reg |= __BIT(13); // enable USART
	usart->CR1 = reg;

	// clear the TC flag fist[Default is 1]
	usart->SR &= ~(__BIT(5) | __BIT(6));
	...
	NVIC_SetPriority(usart_dev->uart_irq, 1);
...
static void usart_deinit(u32 which)
{
	...
	usart_dev->uart->SR = 0; //clear the flag
	...
	switch(which)
	{
		case USART_DEV0:
			...
			usart_dev->dma->IFCR = __BIT(12) | __BIT(13) | __BIT(15);
			break;
		...
		case USART_DEV1:
			...
			usart_dev->dma->IFCR = __BIT(4) | __BIT(5) | __BIT(7);
			break;
...
static usart_rx_block_t *usart_get_rx_block(u32 which)
{
...
void usart_data_post(u32 which, u8* dat, u32 len)
{
	...
	usart_dev->dma_ch->CPAR = (u32)&usart_dev->uart->DR;	//DMA  外设地址 
	...
	switch(dmsg->id)
		{
            ...
            case CM_PC_GET_DeviceID:
               ...
               if(1 == dev->ipc->autotest_flag)
							{						
								 para_read_Nbyte(DEVICE_ID_ADDR, Device_ID,DEVICE_ID_LEN);
...
void comm_uart_init(device_info_t *dev)
{
	...
	usart_init(dev, which, 576000, USART_FMT_8N1);
...
bool comm_message_poll(device_info_t *dev, message_t *msg)
{
	...
	if(FALSE == usart_find_head(rb, writep, 0xff))
	{
		goto exit0;
	}
