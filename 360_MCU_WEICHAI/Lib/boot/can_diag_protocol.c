...
void init_diag_protocol_parameer_boot(void)
{
	...
	client_application_layer_tim.S3_client=5000*OS_1MS;
...
//
u32 SecuriyAlgorithmFBL(u8 *ipSeedArray,u32 ECU_boot_mask)
{
	...
	seed[0] = ipSeedArray[3];
	seed[1] = ipSeedArray[2];
	seed[2] = ipSeedArray[1];
	seed[3] = ipSeedArray[0];
...
*Time ： 20180524
*/
void SecuriyAlgorithmApp(u32 *seed,u32 *key, u32 MASK ,u32 len)
{	
	u32 i;
 	u8 mask[4] = {0x00};
...
u8  SID_10_fun(device_info_t *dev,u8 *buffer,u16 len)
{
	...
	switch(buffer[0])//数据内容  第一个字节buffer[0]代表子服务选项
	{
		...
		case 0x02:
			...
			diag.address_type =DIAG_P_ADDRESS;
...
u8 SID_27_fun(device_info_t *dev,u8 *buffer,u16 len)
{
	u8 send_data[8]={0};
	u8 responce_result=NO_ERR;
	u32 PassWord;  //received key of diagnose tester 诊断仪根据种子返回的Key4个字节             	
	u32 Security_key_boot;
	...
	case 0x01:	//app程序 ：01 02 在扩展会话 物理寻址才 可用			  	   							   						
		...
		{
           	 //0> OSTimeGet(&os_err) test  随机数初始化播种子 
            diag.security_seed = 1;//(uint32_t)(rand());	
	...
	//	dbg_msgv(dev,"PassWord  = %08x\r\n",(u32*)PassWord);
		...
		SecuriyAlgorithmApp(&Seed_APP[0],&Security_key[0],ECU_Mask_APP,1);
		//dbg_msgv(dev,"Security_key = %08x\r\n",Security_key[0]);
		//compare the PassWord with Security_key
		if(PassWord != Security_key[0])  
		{ 
            diag.SAFC_1++;
            if(diag.SAFC_1 <= 3)
            { 
	...
	case 0x21:	//app程序 ：01 02 在扩展会话 物理寻址才 可用	
		...
		else
		{
			...
			send_data[3]=diag.security_seed[0];//int 转u8
	...
	case 0x22://返回 key 
		...
		//dbg_msgv(dev,"PassWord  = %08x\r\n",PassWord);
		//ECU-self calculate the Security_key 
		Security_key_boot = SecuriyAlgorithmFBL(diag.security_seed,ECU_Mask_Boot);
