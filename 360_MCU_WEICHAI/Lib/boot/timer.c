...
/***********************
**@brief: 通用定时器初始化
** which 表示TIMx（1为TIM2 2 为 TIM3）   ms为定时时间 
***
*****************************************/
static void timer_init(device_info_t *dev, u32 which, u16 ms)
{
	u32 prio, reg;
	u32 clock;
	IRQn_Type irq;
	...
	NVIC_DisableIRQ(irq);//使能

	// configs, up-counting
	tm->CR1 = 0;//使能,tm为TIMx寄存器地址

	// auto-reload value
	tm->ARR = ms*10-1; //自动重装值（1ms更新时间）  16位  最大 ms=6550
	
	// prescaler
	tm->PSC = clock /10000 - 1;//分频系数 ：10KHZ
	
	// clear update pending flag,为了防止一开UIE就进入中断函数。必须：URS为1 同时 SR清零在最开始位置
	tm->SR = 0;//清除 中断标志位

	// software UPDATE event, flush config to shadow registers
	tm->EGR =(u16) __BIT(0);//该位由软件设置，由硬件自动清除。重新初始化定时器的计数器并产生对寄存器的更新
...
***
*****************************************/
 static void timer_start(u32 which)
{
	IRQn_Type irq;
	TIM_TypeDef *tm;
	...
	tm->DIER = __BIT(0);//使能  允许更新中断必须放在最后，不然会在打开定时器时立刻进入定时中断
 ...
 ***************************************/
 ...
 void TIM2_IRQHandler(void)
 {
	...
	ARM_ERR_DTC_TIM++;
	if(ARM_ERR_DTC_TIM>=0x04)
	{	
		ARM_ERR_DTC_TIM=0;
		dev->ipc->dtc_arm_com_sate=(dev->ci->host_ready)?0:1;

		if(dev->ipc->usart_normal)
		{
	 		dbg_msg(dev,"comm to arm normal\r\n");
	 		}
		else{
			dbg_msg(dev,"comm to arm err,can`t update arm by mcu\r\n");	
		}
		
		if(dev->ipc->dtc_arm_com_sate==1)
		{
	 		dbg_msg(dev,"record arm err dtc\r\n");
	 	 }
		dev->ci->arm_sleep=1;
...
void TIM3_IRQHandler(void)/*定时器最大定时6500ms*/
{
	...
	if(dev->ci->BusOffSlow==0x01)
	{
		dev->ci->BusOffSlow=0x02;
	}
	
	TIM3->SR = 0;//状态寄存器，硬件置1 软件清零s
...
*
*****************************************/
//static u32 can_tx_timeout=0;
//static u32 can_tx_timerecovery=0;
...
//	dbg_msgv(dev, "dev->ipc->heart_count=%d\r\n",dev->ipc->heart_count);//重启
	if(dev->ci->host_ready)//host发送准备好
	{
		...
		if(dev->ipc->heart_count >= 50)//达到500次
		{
...
void systick_start(device_info_t *dev)
{
	...
	SysTick_Config(dev->clock.hclk / SYSTICK_DIV);//10ms中断一次
}
...
{
	timer_init(get_device_info(), GPIO_TIMER_DEV, TIMER_INTERVAL);
}

void gpio_timer_deinit(void)
{
	timer_deinit(get_device_info(), GPIO_TIMER_DEV);
}
#endif
