...
enum
{
	iocfg_o_pushpull = 0x01, /* max: 10MHz */
	iocfg_o_opendrain = 0x05, /* max: 10MHz */
	iocfg_o_af_pushpull = 0x0b, /* alternate func, max: 10MHz[0x09], 2MHz[0x0a], 50MHz[0x0b] */
	iocfg_o_af_opendain = 0x0f,/* alternate func, max: 10MHz[0x0d], 2MHz[0x0e], 50MHz[0x0f] */
	iocfg_i_analog = 0x00,
	iocfg_i_floating = 0x04,
	iocfg_i_pullxx = 0x08 /* input with internal pullup/pulldown */
};

#define __IO_INIT(gpio, bit, config)	*(&(gpio)->CRL + ((bit) >> 3)) &= ~(0x0fu << (((bit) & 7) << 2));	\
									*(&(gpio)->CRL + ((bit) >> 3)) |= (config) << (((bit) & 7) << 2)

#define IO_INIT_O_PP(gpio, bit, stat)	__IO_INIT(gpio, bit, iocfg_o_pushpull); IO_SET(gpio, bit, stat)
#define IO_INIT_O_OD(gpio, bit, stat)	__IO_INIT(gpio, bit, iocfg_o_opendrain); IO_SET(gpio, bit, stat)
#define IO_INIT_O_AF_PP(gpio, bit)		__IO_INIT(gpio, bit, iocfg_o_af_pushpull)
#define IO_INIT_O_AF_OD(gpio, bit)		__IO_INIT(gpio, bit, iocfg_o_af_opendain)

#define IO_INIT_I_ANA(gpio, bit)		__IO_INIT(gpio, bit, iocfg_i_analog)
#define IO_INIT_I_FLT(gpio, bit)		__IO_INIT(gpio, bit, iocfg_i_floating)
#define IO_INIT_I_PU(gpio, bit)		__IO_INIT(gpio, bit, iocfg_i_pullxx); IO_SET(gpio, bit, HIGH)
#define IO_INIT_I_PD(gpio, bit)		__IO_INIT(gpio, bit, iocfg_i_pullxx); IO_SET(gpio, bit, LOW)

#define IO_SET(gpio, bit, stat)			(gpio)->BSRR = __BIT((bit) + (!(stat) << 4))
#define IO_GET(gpio, bit)				((gpio)->IDR >> (bit) & 1)
...
// max msg para count
#define MSG_MAX_PARA			0xFF//0x80原始值
...
#define SYSTICK_CNT_MAX			6000 // local_counter max
...
typedef struct ipc_info
{
	...
	u32 work_state;//1 休眠  2工作 3 5s无心跳 通讯故障
	u32 avm_disviewsts;
...
typedef struct ctrl_info
{
	...
	u8 write_flash_time_flag;// 写flash记录标志, 1表示已经写了
...
#define _TO_CNT_10MS		((u32)1) // base unit systick  10ms  中断一次
...
typedef void (*state_func_t)(device_info_t *dev, message_t *msg, message_t *dmsg);/*定义函数指针类型  state_func_t*/

#endif /* end of __MISCDEF_H__ */
