...
// 1) Chip select
//#define CORE_36MHZ // For 36MHz HCLK config
...
#define SRAM_SIZE				(0x400ul * 20) /* STM32F103RB has 20KB */
#define SRAM_SIZE_MASK	((0x400ul << 5) - 1)	//	((0x400ul << 5) - 1)
...
//#endif
#define FLASH_SIZE				(64ul * 1024) /* STM32F103C8 has 64KB flash */
#define BOOT_CODE_BASE			FLASH_BASE
#define APP_FLAG_CODE			C4_2_U32('A', 'R', 'M', 'C') // app valid flag, 1.x.x version not compatible
#define APP_CODE_BASE			(BOOT_CODE_BASE + 0x6400ul) // Boot code size: 20KB(include flag page area)
...
#define ENABLE_INTERNAL_WWDG		0 // don't use, for interval too short Ê±¼äÌ«¶Ì
// 7) Mach-type & uuid
