...
//	watchdog_start(dev);
	...
	dbg_msgv(dev, "[Bootloader %d.%d.%d]", get_boot_version() >> 16,
		(get_boot_version() >> 8) & 0xff, get_boot_version() & 0xff);
		
	dbg_msgv(dev, "  [APP %d.%d.%d]", get_app_version() >> 16,
		(get_app_version() >> 8) & 0xff, get_app_version() & 0xff);

	dump_post_message(dev); // Power on self test  芯片参数自测
